...
static mActor_name_t l_tree_max_table[mAGrw_FRUIT_NUM] = {
    TREE_APPLE_FRUIT, TREE_CHERRY_FRUIT, TREE_PEAR_FRUIT, TREE_PEACH_FRUIT, TREE_ORANGE_FRUIT,
};
...
static int mAGrw_CheckPutXmasTree(lbRTC_time_c* time) {
    ...
    if (time->month == lbRTC_DECEMBER && time->day >= 11 && time->day <= 25) {
...
extern void mAGrw_RepairErrorDeposit(void) {
    if (Save_Get(scene_no) == SCENE_FG) {
        int bx;
        int bz;
        int bx_max;
        ...
        bx_max = mFI_GetBlockXMax() - 1;
        bz_max = mFI_GetBlockZMax() - 1;

        for (bz = 1; bz < bz_max; bz++) {
            for (bx = 1; bx < bx_max; bx++) {
                fg_p = mFI_BkNum2UtFGTop_field(bx, bz);
                if (fg_p != NULL) {

                    for (uz = 0; uz < UT_Z_NUM; uz++) {
                        for (ux = 0; ux < UT_X_NUM; ux++) {
                            if (ITEM_NAME_GET_TYPE(*fg_p) == NAME_TYPE_ITEM0) {
                                if (mFI_BkUtNum2DepositGet(bx, bz, ux, uz) == TRUE) {
