...
static void mPC_passwordChk_ovl_move(Submenu* submenu) {
    static mSM_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = { &mPC_move_Move, (mSM_MOVE_PROC)none_proc1,
                                                             (mSM_MOVE_PROC)none_proc1, &mPC_move_Obey, &mPC_move_End };
...
static void mPC_set_character(Submenu* submenu, GAME* game, f32 x, f32 y) {
    // あいことばをいれてね
    static u8 title_str[] = { CHAR_PP_000, CHAR_PP_001, CHAR_PP_009, CHAR_PP_019, CHAR_PP_246,
                              CHAR_PP_194, CHAR_PP_001, CHAR_PP_126, CHAR_PP_018, CHAR_PP_023 };
    // key symbol
    static u8 str_key[] = { CHAR_CONTROL_CODE, mFont_CONT_CODE_FONT_KIGOU, 5 };
    ...
    /* Draw title text */
    mFont_SetLineStrings(game, title_str, sizeof(title_str), 123.0f + x, 33.0f - y, 255, 255, 255, 255, FALSE,
                         FALSE, 1.0f, 1.0f, mFont_MODE_POLY);

    text_x = 65.0f + x;

    /* Draw title text */
    mFont_SetLineStrings(game, str_key, sizeof(str_key), text_x, 63.0f - y, 135, 135, 80, 255, FALSE, FALSE, 1.0f, 1.0f,
                         mFont_MODE_POLY);

    /* Draw first line */
    mFont_SetLineStrings(game, passwordChk_ovl->line0, mPC_STR_SIZE, text_x, 81.0f - y, 235, 235, 50, 255, FALSE, FALSE,
                         1.0f, 1.0f, mFont_MODE_POLY);

    /* Draw second line */
    mFont_SetLineStrings(game, passwordChk_ovl->line1, mPC_STR_SIZE, text_x, 99.0f - y, 235, 235, 50, 255, FALSE, FALSE,
                         1.0f, 1.0f, mFont_MODE_POLY);
    ...
    width = mFont_GetStringWidth(sel_line_p, editor_ovl->cursor_col, FALSE);
...
extern void mPC_passwordChk_ovl_construct(Submenu* submenu) {
    ...
    if (overlay->passwordChk_ovl == NULL) {
        ...
        mem_copy(overlay->passwordChk_ovl->line1, (u8*)overlay->menu_info[mSM_OVL_PASSWORDCHK].data2 + mPC_STR_SIZE,
                 mPC_STR_SIZE);
    }
