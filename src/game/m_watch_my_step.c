        ...
        if (S_watch_my_step.mode != 0) {
            ...
            if (window_item != EMPTY_NO) {
                ...
                if (S_watch_my_step.item_no != window_item || S_watch_my_step.item_no == EMPTY_NO) {
                    ...
                    memset(S_watch_my_step.item_name, CHAR_PP_032, mIN_ITEM_NAME_LEN);
                    if (IS_ITEM_LOST_ITEM(window_item)) {
                        static u8 str[] = { CHAR_PP_004, CHAR_PP_019, CHAR_PP_011, CHAR_PP_091, CHAR_PP_024 };

                        memcpy(S_watch_my_step.item_name, str, sizeof(str));
                    } else {
                ...
                size = (f32)(mFont_GetStringWidth(S_watch_my_step.item_name, len, TRUE)) - 24.0f;

                if (size < 0.0f) {
                    size = 0.0f;
                }

                S_watch_my_step.scale = size / 96.0f;
            } else {
                S_watch_my_step.item_no = EMPTY_NO;
            }
...
extern void watch_my_step_draw(GAME_PLAY* play) {
...

    {
        ...
        switch (S_watch_my_step.mode) {
            case 3: {
                Matrix_push();
                Matrix_scale(S_watch_my_step.opacity * (S_watch_my_step.scale * 0.666667f + 0.333333f),
            ...
            case 4: {
                ...
                Matrix_push();
                Matrix_scale(S_watch_my_step.scale * 0.666667f + 0.333333f, 1.0f, 1.0f, MTX_MULT);
    ...
    if (S_watch_my_step.mode >= 3) {
        ...
        if (text_opacity > 0.0f) {
            mFont_SetLineStrings(
                ...
                (S_watch_my_step.pos_x + 107.0f) + 43.0f * (1.0f - S_watch_my_step.scale),
                113.0f + S_watch_my_step.pos_y,
...
extern Gfx cam_win_model_before[];
...
extern Gfx elc_win_crossT_model[];
...
extern void navigate_camera_draw(GAME_PLAY* play) {
    ...
    if (S_navigate.draw_type != 0) {
        ...
        gSPMatrix(font_gfx++, _Matrix_to_Mtx_new(g), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(font_gfx++, cam_win_model_before);
        ...
        if (S_navigate.draw_type == 2) {
            ...
            gDPSetPrimColor(font_gfx++, 0, 255, 130, 120, 140, a);
            gSPDisplayList(font_gfx++, elc_win_crossT_model);
...
extern void mWt_mybell_confirmation_draw(GAME_PLAY* play) {
    ...
    if (S_mybell_conf.draw_type != 0) {
        ...
        gDPSetRenderMode(font_gfx++, G_RM_PASS, G_RM_XLU_SURF2);
        ...
        /* Draw bell amount */
        {
            ...
            mFont_UnintToString(bell_str, 6, S_mybell_conf.all_money, 6, FALSE, FALSE, FALSE);
            mFont_SetLineStrings(game, bell_str, 6, 214.0f, 48.0f,
                                 255, 245, 0, a, FALSE, FALSE, 0.75f, 0.75f, mFont_MODE_FONT);
        }
    }
}
