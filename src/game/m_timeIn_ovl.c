...
static void mTI_set_time(Submenu* submenu) {
    ...
    if ((old_min < min && min <= now_min) || (old_min > now_min && (old_min < min || min <= now_min))) {
...
static void mTI_window_close(Submenu* submenu, mSM_MenuInfo_c* menu_info) {
    ...
    if (menu_info->data0 == mTI_OVL_OPEN_INITIAL) {
        ...
        if (set_time.year != timeIn_ovl->original_time.year || set_time.month != timeIn_ovl->original_time.month ||
            set_time.day != timeIn_ovl->original_time.day || set_time.hour != timeIn_ovl->original_time.hour ||
    ...
    if (timeIn_ovl->input_disabled_flag == TRUE) {
        ...
        if (timeIn_ovl->sel_idx != mTI_IDX_OK) {
            ...
            if (lbRTC_IsValidTime(&time) == FALSE) {
                ...
                } else if ((sel_idx == mTI_IDX_MONTH && values[mTI_IDX_MONTH] > 0 && values[mTI_IDX_MONTH] < 13) ||
...
static void mTI_timeIn_ovl_move(Submenu* submenu) {
    static mSM_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = {
        &mTI_move_Move, &mTI_move_Play, (mSM_MOVE_PROC)&none_proc1, (mSM_MOVE_PROC)&none_proc1, &mTI_move_End,
    };
...
extern Gfx tim_win_youbiT_model[];
extern u8 tim_win_sun_tex_rgb_ia8[];
extern u8 tim_win_mon_tex_rgb_ia8[];
extern u8 tim_win_tue_tex_rgb_ia8[];
extern u8 tim_win_wed_tex_rgb_ia8[];
extern u8 tim_win_thu_tex_rgb_ia8[];
extern u8 tim_win_fri_tex_rgb_ia8[];
extern u8 tim_win_sat_tex_rgb_ia8[];
static u8* tim_win_anime_tex_table[] = {
    tim_win_sun_tex_rgb_ia8, tim_win_mon_tex_rgb_ia8, tim_win_tue_tex_rgb_ia8, tim_win_wed_tex_rgb_ia8,
    tim_win_thu_tex_rgb_ia8, tim_win_fri_tex_rgb_ia8, tim_win_sat_tex_rgb_ia8,
};
...
static void mTI_set_frame_dl(Submenu* submenu, GRAPH* graph, f32 x, f32 y) {
    ...
    week = lbRTC_Week(GAME_YEAR_MIN + timeIn_ovl->values[mTI_IDX_YEAR], timeIn_ovl->values[mTI_IDX_MONTH],
                      timeIn_ovl->values[mTI_IDX_DAY]);
    ...
    gSPDisplayList(POLY_OPA_DISP++, tim_win_mode);
    ...
    s = (int)(-submenu->overlay->menu_control.texture_pos[0] * 4.0f) & 0xFF;
    t = (int)(-submenu->overlay->menu_control.texture_pos[1] * 4.0f) & 0xFF;
    gDPSetTileSize_Dolphin(POLY_OPA_DISP++, G_TX_RENDERTILE, s, t, 32, 32);

    /* Draw window background */
    gSPDisplayList(POLY_OPA_DISP++, tim_win_model);

    if (week == lbRTC_SUNDAY) {
        gDPSetPrimColor(POLY_OPA_DISP++, 0, 255, 255, 225, 225, 255);
        gDPSetEnvColor(POLY_OPA_DISP++, 195, 85, 85, 255);
    } else {
        gDPSetPrimColor(POLY_OPA_DISP++, 0, 255, 225, 255, 255, 255);
        gDPSetEnvColor(POLY_OPA_DISP++, 110, 115, 195, 255);
    }
    gSPSegment(POLY_OPA_DISP++, G_MWO_SEGMENT_8, tim_win_anime_tex_table[week]);
    gSPDisplayList(POLY_OPA_DISP++, tim_win_youbiT_model);
...
static void mTI_set_clock(Submenu* submenu, GRAPH* graph, f32 x, f32 y) {
    static Gfx* displayList_table[] = { tim_hari_b2T_model, tim_hari_b1T_model, tim_hari_b3T_model };
...
static void mTI_set_character(Submenu* submenu, GAME* game, f32 x, f32 y) {
    static u8 chr_table[] = { CHAR_PP_237, CHAR_PP_005, CHAR_PP_195, CHAR_PP_194, CHAR_PP_000,
                              CHAR_PP_193, CHAR_PP_013, CHAR_PP_018, CHAR_PP_023 };
    static u8 chr_table4[] = { CHAR_PP_004, CHAR_PP_193, CHAR_PP_124 };
    static f32 set_pos[mTI_IDX_NUM][2] = {
        { 100.0f, 115.0f }, { 143.0f, 115.0f }, { 186.0f, 115.0f }, { 92.0f, 139.0f }, { 125.0f, 139.0f }
    };
    static int col_type0[] = { 255, 0, 0 };
    static int col_type1[] = { 60, 90, 255 };
    u8 str[2];
    int* col;
    int i;
    mTI_Ovl_c* timeIn_ovl = submenu->overlay->timeIn_ovl;
    f32* set_pos_p = &set_pos[0][0];
    mFont_SetLineStrings(game, chr_table, sizeof(chr_table), (x + 133.0f) + 2.0f, (83.0f - y) - 4.0f, 255, 255, 255,
                         255, FALSE, FALSE, 0.875f, 0.875f, mFont_MODE_POLY);
    for (i = 0; i < mTI_IDX_NUM; i++, set_pos_p += 2) {
        mFont_UnintToString(str, 2, timeIn_ovl->values[i], 2, FALSE, TRUE, FALSE);
        col = col_type1;
        if (i == timeIn_ovl->sel_idx) {
            col = col_type0;
        }
        mFont_SetLineStrings(game, str, 2, x + set_pos_p[0], -y + set_pos_p[1], col[0], col[1], col[2], 255, FALSE,
                             FALSE, 0.875f, 0.875f, mFont_MODE_POLY);
    }
    col = col_type1;
    if (timeIn_ovl->sel_idx == mTI_IDX_OK) {
        col = col_type0;
    }
    mFont_SetLineStrings(game, chr_table4, sizeof(chr_table4), x + 176.0f, 144.0f - y, col[0], col[1], col[2], 255,
                         FALSE, FALSE, 0.875f, 0.875f, mFont_MODE_POLY);
}
...
static void mTI_timeIn_ovl_init(Submenu* submenu) {
    ...
    timeIn_ovl->sel_idx = mTI_IDX_YEAR;
    timeIn_ovl->input_disabled_flag = FALSE;
