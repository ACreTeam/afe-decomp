...
static void aEGH_actor_ct(ACTOR* actorx, GAME* game) {
    ...
    if (NPC_CLIP->birth_check_proc(actorx, game) == TRUE) {
        ...
        if (ghost_save == NULL) {
            ...
            if (rtc_time->year != ghost_save->renew_time.year || rtc_time->month != ghost_save->renew_time.month ||
...
static void aEGH_actor_dt(ACTOR* actorx, GAME* game) {
    mEv_common_data_c* event_common = Common_GetPointer(event_common);

    event_common->exist_flags0 &= ~1;
