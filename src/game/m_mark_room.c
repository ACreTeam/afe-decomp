...
static int mMkRm_NoMarkLetter(int arrange_idx, int handbill_no) {
  ...
  if (free_idx >= 0) {
    ...
    mMl_clear_mail(&mail);
    mHandbill_Load_HandbillFromRom(mail.content.text.split.header, &header_back_start, mail.content.text.split.footer, mail.content.text.split.body, handbill_no);
