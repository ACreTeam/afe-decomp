...
static void graph_setup_double_buffer(GRAPH* this) {
    ...
    CONSTRUCT_THA_GA(&this->new2_thaga, new2, NEW2);
...
static DLFTBL_GAME* game_get_next_game_dlftbl(GAME* game) {
    int dlf_no = game_get_next_game_dlf_no(game);
    
    if (dlf_no >= 0 && dlf_no < GAME_DLFTBL_NUM) {
        return &game_dlftbls[dlf_no];
    }
...
static int graph_draw_finish(GRAPH* this) {
    ...
    gSPBranchList(NOW_WORK_DISP++, this->Gfx_list10); // work -> bg_opa
    gSPBranchList(NOW_BG_OPA_DISP++, this->Gfx_list08); // bg_opa -> shadow
    gSPBranchList(NOW_SHADOW_DISP++, this->Gfx_list11); // shadow -> bg_xlu
    gSPBranchList(NOW_BG_XLU_DISP++, this->Gfx_list00); // bg_xlu -> poly_opa
    gSPBranchList(NOW_POLY_OPA_DISP++, this->Gfx_list01); // poly_opa -> poly_xlu
    gSPBranchList(NOW_POLY_XLU_DISP++, this->Gfx_list09); // poly_xlu -> light
    gSPBranchList(NOW_LIGHT_DISP++, this->Gfx_list07); // light -> font
    gSPBranchList(NOW_FONT_DISP++, this->Gfx_list12); // font -> new2
    gSPBranchList(NOW_NEW2_DISP++, this->Gfx_list04); // new2 -> overlay
    ...
    if (!SYSDYNAMIC_CHECK_START()) {
        _dbg_hungup(__FILE__, 449);
    }

    if (!SYSDYNAMIC_CHECK_END()) {
        err = TRUE;
        _dbg_hungup(__FILE__, 457);
    }
    SYSDYNAMIC_CLOSE();
    ...
    if (THA_GA_isCrash(&this->new2_thaga)) {
        err = TRUE;
    }
...
extern void graph_proc(void* arg) {
    ...
    DLFTBL_GAME* dlftbl = &game_dlftbls[GAME_DLFTBL_FIRST_GAME];
    int dlf_no = GAME_DLFTBL_FIRST_GAME;
    ...
    while (dlftbl != NULL) {
        ...
        game_ct(game, dlftbl->init, __graph, dlf_no);
        emu64_refresh();
