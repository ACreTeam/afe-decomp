#include "types.h"
#include "libforest/gbi_extensions.h"
#include "PR/gbi.h"

Vtx obj_s_crackstoneE_v[] ATTRIBUTE_ALIGN(32) = {
#include "assets/forestd/obj_s_crackstoneE/obj_s_crackstoneE_v.inc"
};

Gfx obj_s_crackstoneE_gfx_model[] ATTRIBUTE_ALIGN(8) = {
gsSPVertex(obj_s_crackstoneE_v, 15, 0),
gsSPNTrianglesInit_5b(17, 0, 1, 2, 0, 3, 1, 4, 3, 5),
gsSPNTriangles_5b(4, 1, 3, 3, 6, 5, 3, 0, 6, 4, 7, 1),
gsSPNTriangles_5b(7, 2, 1, 8, 6, 9, 6, 0, 9, 10, 11, 12),
gsSPNTriangles_5b(10, 13, 11, 9, 14, 10, 9, 0, 14, 14, 13, 10),
gsSPNTriangles_5b(8, 9, 12, 9, 10, 12, 0, 0, 0, 0, 0, 0),
gsSPEndDisplayList(),
};
