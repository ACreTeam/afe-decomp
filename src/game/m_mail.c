...
extern void mMl_clear_mail(Mail_c* mail) {
    ...
    mem_clear(mail->content.text.all, sizeof(mail->content.text.all), CHAR_SPACE);
    mail->content.font = -1;
}
...
static void mMl_load_mail_data2(Mail_c* mail, int mail_no) {
    ...
    bcopy(header, mail->content.text.split.header, MAIL_HEADER_LEN);
    bcopy(footer, mail->content.text.split.footer, MAIL_FOOTER_LEN);
    mail->content.header_back_start = header_back_start;
}
