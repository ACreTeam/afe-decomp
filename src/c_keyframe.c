...
extern int cKF_SkeletonInfo_R_play(cKF_SkeletonInfo_R_c* keyframe) {
    ...
    if (keyframe->rotation_diff_table != NULL) {
        ...
        if (F32_IS_ZERO(keyframe->morph_counter)) {
            currentJointPtr = keyframe->current_joint;
        } else {
...
extern void cKF_SkeletonInfo_R_combine_translation(s16** joint, int* flag, cKF_SkeletonInfo_R_combine_work_c* combine,
    ...
    int i;
    int j;
    cKF_SkeletonInfo_R_combine_work_c* work_tbl[3];

    for (i = 0; i < combine_cnt; i++) {
        work_tbl[i] = &combine[i];
    }

    for (i = 0; i < 3; i++) {
        /* Determine which animation we should pull from for the joint */
        cKF_SkeletonInfo_R_combine_work_c* target_p = work_tbl[*part_table];

        if (target_p->anm_check_bit_tbl[0] & *flag) {
            (**joint) =
                cKF_KeyCalc(target_p->anm_data_src_idx, target_p->anm_key_num[target_p->anm_key_num_idx],
                            target_p->anm_data_src, target_p->keyframe->frame_control.current_frame);
        } else {
            (**joint) = target_p->anm_const_val_tbl[target_p->anm_const_val_tbl_idx];
        }

        for (j = 0; j < combine_cnt; j++) {
            cKF_SkeletonInfo_R_combine_work_c* target_p = work_tbl[j];

            if (target_p->anm_check_bit_tbl[0] & *flag) {
                target_p->anm_data_src_idx += target_p->anm_key_num[target_p->anm_key_num_idx];
                target_p->anm_key_num_idx++;
            } else {
                target_p->anm_const_val_tbl_idx++;
            }
        }
...
extern void cKF_SkeletonInfo_R_combine_rotation(s16** joint, int* flag, cKF_SkeletonInfo_R_combine_work_c* combine,
    ...
    for (i = 0; i < work_tbl[0]->keyframe->skeleton->num_joints; i++) {
        *flag = 4;

        for (j = 0; j < 3; j++) {
            /* Determine which animation we should pull from for the joint */
            cKF_SkeletonInfo_R_combine_work_c* target_p = work_tbl[part_table[i + 1]];

            if (*flag & target_p->anm_check_bit_tbl[i]) {
                (**joint) = 
                    cKF_KeyCalc(target_p->anm_data_src_idx, target_p->anm_key_num[target_p->anm_key_num_idx],
                                target_p->anm_data_src, target_p->keyframe->frame_control.current_frame);
            } else {
            ...
            for (k = 0; k < combine_cnt; k++) {
                cKF_SkeletonInfo_R_combine_work_c* target_p = work_tbl[k];
...
extern int cKF_SkeletonInfo_R_combine_play(cKF_SkeletonInfo_R_c* info1, cKF_SkeletonInfo_R_c* info2, s8* part_table) {
    ...
    if(F32_IS_ZERO(info1->morph_counter)) {
        joint = &info1->current_joint->x;
    } else {
        joint = &info1->target_joint->x;
    }
    ...
    combinet = 0x20;
    cKF_SkeletonInfo_R_combine_translation(&joint, &combinet, &combine[0], part_table, 2);
    cKF_SkeletonInfo_R_combine_rotation(&joint, &combinet, &combine[0], part_table, 2);
...
extern void cKF_SkeletonInfo_R_T_combine_play(int* state1, int* state2, int* state3, cKF_SkeletonInfo_R_c* info1,
    ...
    combinet = 0x20;
    cKF_SkeletonInfo_R_combine_translation(&joint, &combinet, &combine[0], part_table, 3);
    cKF_SkeletonInfo_R_combine_rotation(&joint, &combinet, &combine[0], part_table, 3);
...
static int cKF_SkeletonInfo_R_Multi_combine_play_chk(cKF_SkeletonInfo_R_c** info_p, s8* part_table, int count) {
    int ret = TRUE;

    if (part_table == NULL || count > 4) {
        ret = FALSE;
    } else {
        int i;
...
extern void cKF_SkeletonInfo_R_Multi_combine_play(int* state_tbl, cKF_SkeletonInfo_R_c** info_tbl, s8* part_table, int count) {
    cKF_SkeletonInfo_R_combine_work_c combine[4];
    int flag;
    s16* joint;
    int i;

    if (cKF_SkeletonInfo_R_Multi_combine_play_chk(info_tbl, part_table, count)) {
        if (F32_IS_ZERO(info_tbl[0]->morph_counter)) {
            joint = &info_tbl[0]->current_joint->x;
        } else {
            joint = &info_tbl[0]->target_joint->x;
        }

        for (i = 0; i < count; i++) {
            cKF_SkeletonInfo_R_combine_work_set(&combine[i], info_tbl[i]);
        }

        flag = cKF_ANIMATION_BIT_TRANS_X;
        cKF_SkeletonInfo_R_combine_translation(&joint, &flag, combine, part_table, count);
        cKF_SkeletonInfo_R_combine_rotation(&joint, &flag, combine, part_table, count);
        if (info_tbl[0]->rotation_diff_table != NULL) {
            s_xyz* applyjoint;

            if (F32_IS_ZERO(info_tbl[0]->morph_counter)) {
                applyjoint = info_tbl[0]->current_joint;
            } else {
                applyjoint = info_tbl[0]->target_joint;
            }
            
            applyjoint += 1; // skip root translation joint
            for (i = 0; i < info_tbl[0]->skeleton->num_joints; i++) {
                applyjoint->x += info_tbl[0]->rotation_diff_table[i].x;
                applyjoint->y += info_tbl[0]->rotation_diff_table[i].y;
                applyjoint->z += info_tbl[0]->rotation_diff_table[i].z;
                applyjoint++;
            }
        }
        ...
        if (F32_IS_ZERO(info_tbl[0]->morph_counter)) {
            for (i = 0; i < count; i++) {
                state_tbl[i] = cKF_FrameControl_play(&info_tbl[i]->frame_control);
            }
        } else {
            if (info_tbl[0]->morph_counter > 0.0f) {
                cKF_SkeletonInfo_R_morphJoint(info_tbl[0]);
                info_tbl[0]->morph_counter -= 0.5f;
                if (info_tbl[0]->morph_counter <= 0.0f) {
                    info_tbl[0]->morph_counter = 0.0f;
                }

                for (i = 0; i < count; i++) {
                    state_tbl[i] = cKF_STATE_NONE;
                }
            } else {
                cKF_SkeletonInfo_R_morphJoint(info_tbl[0]);
                info_tbl[0]->morph_counter += 0.5f;
