...
#define mNpcW_MAKE_SCHEDULE_TABLE(goal_data) { ARRAY_SIZE(goal_data, mNpcW_GoalData_c), goal_data }

static u8 l_p0a0h1_walk_goal[1] = {
    mNpcW_GOAL_HOME,
};

static u8 l_p1a1h1_walk_goal[3] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_ALONE, mNpcW_GOAL_HOME,
};

static u8 l_p1a0h1_walk_goal[2] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_HOME,
};

static u8 l_p0a1h0_walk_goal[1] = {
    mNpcW_GOAL_ALONE,
};

static u8 l_p0a1h1_walk_goal[2] = {
    mNpcW_GOAL_ALONE, mNpcW_GOAL_HOME,
};

static u8 l_p1a2h1_walk_goal[4] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE, mNpcW_GOAL_HOME,
};

static u8 l_p2a1h2_walk_goal[5] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_ALONE, mNpcW_GOAL_HOME, mNpcW_GOAL_HOME,
};

static u8 l_p3a7h0_walk_goal[10] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE,
    mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE,
};

static u8 l_p7a3h0_walk_goal[10] = {
    mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_SHRINE, mNpcW_GOAL_ALONE,
    mNpcW_GOAL_ALONE, mNpcW_GOAL_ALONE,
};

static mNpcW_GoalDataTable_c l_goal_table[10] = {
    {NULL, 0x00000000},
    {l_p0a0h1_walk_goal, ARRAY_COUNT(l_p0a0h1_walk_goal)},
    {l_p1a1h1_walk_goal, ARRAY_COUNT(l_p1a1h1_walk_goal)},
    {l_p1a0h1_walk_goal, ARRAY_COUNT(l_p1a0h1_walk_goal)},
    {l_p0a1h0_walk_goal, ARRAY_COUNT(l_p0a1h0_walk_goal)},
    {l_p0a1h1_walk_goal, ARRAY_COUNT(l_p0a1h1_walk_goal)},
    {l_p1a2h1_walk_goal, ARRAY_COUNT(l_p1a2h1_walk_goal)},
    {l_p2a1h2_walk_goal, ARRAY_COUNT(l_p2a1h2_walk_goal)},
    {l_p3a7h0_walk_goal, ARRAY_COUNT(l_p3a7h0_walk_goal)},
    {l_p7a3h0_walk_goal, ARRAY_COUNT(l_p7a3h0_walk_goal)},
};

static mNPS_schedule_data_c* mNpcW_GetGoalDataInfo(int looks, int now_sec) {
    mNPS_schedule_data_table_c* schedule_table = mNPS_schedule[looks];
    int schedule_num = schedule_table->count;
    mNPS_schedule_data_c* schedule_data_save = schedule_table->sched_data;
    mNPS_schedule_data_c* schedule_data = schedule_data_save;
    int i;

    for (i = 0; i < schedule_num; i++) {
        if (schedule_data->end_time > now_sec) {
            break;
        }
    ...
    schedule_data = &schedule_data_save[schedule_num];
    schedule_data--; /* Return the last schedule data */
    return schedule_data;
}
...
static int mNpcW_DecideNpc(Animal_c* animal, u16 used) {
    ...
    for (i = 0; i < ANIMAL_NUM_MAX; i++) {
        if (mNpc_CheckFreeAnimalPersonalID(&animal->id) == FALSE) {
            mNPS_schedule_data_c* schedule_data;

            exist |= 1 << i;
            schedule_data = mNpcW_GetGoalDataInfo(animal->id.looks, now_sec);

            if (schedule_data->walk_type != 0) {
                possible_goal |= 1 << i;
...
static void mNpcW_SetNpcWalkInfo(mNpcW_info_c* info, Animal_c* animal, int idx) {
    ...
    if (schedule_data->walk_type != 0) {
        mNpcW_GoalDataTable_c* goal_table = &l_goal_table[schedule_data->walk_type];
        info->goal_type = goal_table->data_p[RANDOM(goal_table->count)];
    } else {
...
static void mNpcW_GetBlockXZNumExceptHome(int* goal_block_x, int* goal_block_z, Animal_c* animal) {
    ...
    *goal_block_z = i + 1;
    ...
    if (*goal_block_x <= 0 || *goal_block_x >= BLOCK_X_NUM || *goal_block_z <= 0 ||
        *goal_block_z >= (BLOCK_Z_NUM - 2)) {
        *goal_block_x = 4;
        *goal_block_z = 3;
    }
}
...
static void mNpcW_SetHomeBlockSource(mNpc_walk_c* walk, Animal_c* animal) {
    ...
    if (l_goal_block[mNpcW_GOAL_BLOCK_HOME][0] <= 0 || l_goal_block[mNpcW_GOAL_BLOCK_HOME][0] >= BLOCK_X_NUM ||
...
extern void mNpcW_SetGoalBlock(mNpcW_info_c* info) {
    ...
    if (info->goal_block_x <= 0 || info->goal_block_x >= BLOCK_X_NUM || info->goal_block_z <= 0 ||
        info->goal_block_z >= (BLOCK_Z_NUM - 2)) {
        info->goal_block_x = 4;
        info->goal_block_z = 3;
    }
}
...
extern void mNpcW_InitNpcWalk(mNpc_walk_c* walk) {
    ...
    npc_max = mNpcW_GET_WALK_NUM(mNpc_GetAnimalNum());
