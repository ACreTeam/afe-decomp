...
extern void lbRTC_GetNukegaraTime(lbRTC_time_c* time) {
    OSCalendarTime ctime;
    OSCalendarTime ctime2;
    OSTick tick;
    OSTime t;
    
    // Get castoff time plus our rtc time delta
    t = Save_Get(nukegara_time);
    t += Save_Get(time_delta);
    OSTicksToCalendarTime(t, &ctime);

    // Clamp year
    if (ctime.year > GAME_YEAR_MAX - 2) {
        ctime.year = GAME_YEAR_MAX - 2;
    } else if (ctime.year < GAME_YEAR_MIN + 1) {
        ctime.year = GAME_YEAR_MIN + 1;
    }
    
    // Convert back to ticks and convert to lbRTC_time_c
    OSTicksToCalendarTime(OSCalendarTimeToTicks(&ctime), &ctime2);
    lbRTC_CalenderTimeToRTCTime(&ctime2, time);
}
