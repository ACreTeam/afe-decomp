...
static void aBC_actor_dt(ACTOR*, GAME*);
...
static void aBC_actor_dt(ACTOR* actorx, GAME* game) {
    // nothing
}
...
static void aBC_deleteActor_part(GAME_PLAY* play, int part) {
    ...
    while (TRUE) {
        ...
        if ((check_bx >= 0 && check_bx != last_bx && check_bx != now_bx) &&
...
static int aBC_setupOtherActor(GAME_PLAY* play, mActor_name_t actor_id, s16 profile, f32 pos_x, f32 pos_z,
    ...
    actor = Actor_info_make_actor(&play->actor_info, (GAME*)play, profile, pos.x, pos.y, pos.z, 0, 0, 0,
                                  play->block_table.block_x, play->block_table.block_z, -1, actor_id, actor_id, -1, -1);
        ...
        for (ut_x = 0; ut_x < UT_X_NUM; ut_x++) {
            switch (ITEM_NAME_GET_TYPE(*item_p)) {
                case NAME_TYPE_ITEM2: {
                    ...
                    setup_actor_flag |=
                        aBC_setupOtherActor(play, *item_p, move_obj_profile_table[idx], base_x + aBC_pos_table[ut_x],
                                            base_z + aBC_pos_table[ut_z], EMPTY_NO);
                    break;
                }
                ...
                case NAME_TYPE_PROPS: {
                    ...
                    setup_actor_flag |=
                        aBC_setupOtherActor(play, *item_p, props_profile_table[idx], base_x + aBC_pos_table[ut_x],
                                            base_z + aBC_pos_table[ut_z], clear_item);
                    break;
                }

                case NAME_TYPE_STRUCT:
                    if (Common_Get(clip).structure_clip != NULL) {
                        STRUCTURE_ACTOR* actor = (*Common_Get(clip).structure_clip->setup_actor_proc)(
                            (GAME*)play, *item_p, -1, base_x + aBC_pos_table[ut_x], base_z + aBC_pos_table[ut_z]);
...
static int aBC_setupCommonMvActor(GAME_PLAY* play, mFM_move_actor_c* mv_actor_list, int mv_actor_list_no, s16 profile,
    ...
    if (Actor_info_make_actor(actor_info, (GAME*)play, profile, pos.x, pos.y, pos.z, 0, 0, 0, play->block_table.block_x,
                              play->block_table.block_z, mv_actor_list_no, mv_actor_list->name_id, mv_actor_list->arg,
                              mv_actor_list->npc_info_idx, -1) != NULL) {
        res = TRUE;
    }
        ...
        for (i = 0; i < mFM_MOVE_ACTOR_NUM; i++) {
            if (((mv_actor_bitfield >> i) & 1) == 1) {
                ...
                switch (ITEM_NAME_GET_TYPE(mv_actor_name)) {
                    case NAME_TYPE_ITEM2:
                        was_born = aBC_setupCommonMvActor(play, mv_actor_list_p, i,
                                                          move_obj_profile_table[mv_actor_name - ETC_START],
                                                          base_x + aBC_pos_table[mv_actor_list_p->ut_x],
                                                          base_z + aBC_pos_table[mv_actor_list_p->ut_z]);
                        break;
                    case NAME_TYPE_ACTOR:
                        was_born = aBC_setupCommonMvActor(play, mv_actor_list_p, i,
                                                          actor_profile_table[mv_actor_name - MISC_ACTOR_START],
                                                          base_x + aBC_pos_table[mv_actor_list_p->ut_x],
                                                          base_z + aBC_pos_table[mv_actor_list_p->ut_z]);
                        break;
                    ...
                    case NAME_TYPE_NPC:
                        if (Common_Get(clip).npc_clip != NULL && Common_Get(clip).npc_clip->setupActor_proc != NULL) {
                            was_born = (*Common_Get(clip).npc_clip->setupActor_proc)(
                                play, mv_actor_name, mv_actor_list_p->npc_info_idx, i, mv_actor_list_p->arg,
                                block_table->block_x, block_table->block_z, mv_actor_list_p->ut_x,
                                mv_actor_list_p->ut_z);
                        } else {
                            was_born = FALSE;
                        }
                        break;
...
static void aBC_set_boat(BIRTH_CONTROL_ACTOR* birth_control, GAME_PLAY* play) {
    if (mEv_IsNotTitleDemo() && aBC_chk_near_boat_block(birth_control, play) == TRUE) {
        mActor_name_t* boat_ut_p =
            mFI_UtNum2UtFG(5 * UT_X_NUM + 5, 6 * UT_Z_NUM + 10); // Set boat at F-5, unit 5-10 (x-z)
