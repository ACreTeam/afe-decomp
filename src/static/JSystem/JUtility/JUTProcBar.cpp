...
#define US_PER_60FPS_FRAME 16666 // (1frame / 60fps) * (1000000us/s) (NTSC)
#define US_PER_50FPS_FRAME 20000 // (1frame / 50fps) * (1000000us/s) (PAL)
...
JUTProcBar::JUTProcBar() {
    ...
    mProcessBarMode = PROCESS_BAR_MODE_STACKED_TIMELINE;
    mHeapBarMode = HEAP_BAR_MODE_WITH_WATCH_OVERLAY;
    mWatchHeap = nullptr;
}
...
// Matches
void JUTProcBar::bar_subroutine(int x, int y, int h, int pixelsPerFrame, int frameDurationUs, int currentCostUs,
                                int peakCostUs, JUtility::TColor fillColor, JUtility::TColor peakColor) {
    int curWidthPx = currentCostUs * pixelsPerFrame / frameDurationUs;
    int peakWidthPx = peakCostUs * pixelsPerFrame / frameDurationUs;

    J2DFillBox(x, y, curWidthPx, h, fillColor);
    if (peakWidthPx >= 0) {
        if (peakWidthPx < 6)
            J2DFillBox(x, y, peakWidthPx, h, peakColor);
        else
            J2DFillBox(x + peakWidthPx - 6, y, 6.0f, h, peakColor);
    }
}
...
void JUTProcBar::adjustMeterLength(u32 param_0, f32* param_1, f32 param_2, f32 param_3, int* param_4) {
    ...
    float var1 = *param_1;
    while (var1 > param_2) {
        if (param_0 * var1 * 20.0f / (f32)US_PER_60FPS_FRAME <= mParams.mWidth - 30.0f)
            break;
    ...
    while (!var2 && var1 < param_3) {
        ...
        if (*param_4 < 0x1e)
            break;
        if ((param_0 * var1 * 20.0f / (f32)US_PER_60FPS_FRAME) < (mParams.mWidth - 60.0f))
            var1 += 0.2f;
        break;
    }
    *param_1 = var1;
...
void JUTProcBar::drawProcessBar() {
    if (mVisible) {
        int frameDuration = US_PER_60FPS_FRAME; // duration in miliseconds? for how long a frame takes,
        if (JUTVideo::getManager() &&
            ((JUTVideo::getManager()->getRenderMode()->viTVmode >> 2) & 0x0f) == VI_PAL) // possibly a define
            frameDuration = US_PER_50FPS_FRAME;                                                       // duration for PAL
        ...
        if (mProcessBarMode == PROCESS_BAR_MODE_SEPARATE_WITH_PEAK) {
            int r23 = mParams.mPosY + mParams.mBarWidth;
            bar_subroutine(mParams.mPosX + 1, r23, r26, r28, frameDuration, mGp.mCost, mGp.mPeakCost,
                           JUtility::TColor(80, 255, 80, 255), JUtility::TColor(100, 255, 120, 255));
            r23 += mParams.mBarWidth * 2;
            bar_subroutine(mParams.mPosX + 1, r23, r26, r28, frameDuration, mCpu.mCost, mCpu.mPeakCost,
                           JUtility::TColor(255, 80, 80, 255), JUtility::TColor(255, 100, 100, 255));
            r23 += mParams.mBarWidth * 2;
            bar_subroutine(mParams.mPosX + 1, r23, r26, r28, frameDuration, mIdle.mCost, mIdle.mPeakCost,
        ...
        for (int i = 0; i < 8; i++) {
            CTime* time = &mUsers[i];
            if (++time->mPeakAgeCounter >= 16 || time->mCost > time->mPeakCost) {
                time->mPeakCost = time->mCost;
                time->mPeakAgeCounter = 0;
            }
            if (time->mPeakCost > temp3)
                temp3 = time->mPeakCost;
        }
        if ((temp3 ? true : false) == true) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (time->mCost != 0 || time->mPeakCost != 0) {
                    int temp4 = time->mCost * r21 / frameDuration;
                    int temp5 = time->mPeakCost * r21 / frameDuration;
                    time->mCost = 0;
...
void JUTProcBar::drawHeapBar() {
    if (mHeapBarVisible) {
        ...
        if (mHeapBarMode == HEAP_BAR_MODE_WITH_WATCH_OVERLAY) {
