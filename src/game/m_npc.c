...
/* @BUG - devs checked for memory being NULL *after* deferencing it */
        ...
        if (npc_max > (ids + homes_in_block) && count < ANIMAL_NUM_MAX) {
            ...
            for (j = 0; j < ANIMAL_NUM_MAX; j++) {
                ...
                if (!mNpc_CheckFreeAnimalPersonalID(&animal->id)) {
                    ...
                    if (i != ids) {
                        valid = FALSE;
                    } else if (check_flag == TRUE && animal->home_info.block_x == bx &&
    ...
    extern void mNpc_AnimalMail2Mail(Mail_c * mail, Anmplmail_c * animal_mail, PersonalID_c * pid,
    ...
    extern int mNpc_SendMailtoNpc(Mail_c * mail) {
        ...
        if (mMl_get_npcinfo_from_mail_name(&anm_id, &mail->header.recipient) == TRUE) {
            ...
            if (anm_idx != -1) {
                ...
                memory = NULL;
                memory_idx =
                    mNpc_GetAnimalMemoryIdx(&mail->header.sender.personalID, animal->memories, ANIMAL_MEMORY_NUM);
                ...
                if (mEv_CheckFirstJob() == TRUE) {
                    ...
                    if (first_job != NULL &&
                        ...
                        mNpc_CheckFreeAnimalPersonalID(&first_job->recipient) == FALSE) {
    ...
    static int mNpc_GetHandbillz(Mail_c * mail, int super_num, int mail_a_num, int mail_b_num, int mail_c_num,
                                 int ps_num) {
    ...
    static void mNpc_GetRemailGoodData(Mail_c * mail, PersonalID_c * pid, AnmPersonalID_c * anm_id,
                                       Anmremail_c * remail, u8 foreign) {
    ...
    static void mNpc_GetRemailWrongData(Mail_c * mail, PersonalID_c * pid, AnmPersonalID_c * anm_id,
                                        Anmremail_c * remail, u8 foreign) {
    ...
    static void mNpc_GetRemailData(Mail_c * mail, PersonalID_c * pid, AnmPersonalID_c * anm_id, Anmremail_c * remail,
                                   int cond, u8 foreign) {
    ...
    static void mNpc_LoadMailDataCommon2(Mail_c * mail, PersonalID_c * pid, AnmPersonalID_c * anm_id,
                                         mActor_name_t present, u8 paper_type, int mail_no) {
    ...
    static void mNpc_GetEventPresent(mActor_name_t * present, int type) {
        ...
        static int category_table[mNpc_EVENT_MAIL_FRIEND_NUM] = { mSP_KIND_FURNITURE, mSP_KIND_FURNITURE,
                                                                  mSP_KIND_CLOTH };
    ...
    extern int mNpc_SendVtdayMail() {
        ...
        for (i = 0; i < ANIMAL_NUM_MAX; i++) {
            ...
            if (mNpc_CheckFreeAnimalPersonalID(&animal_p->id) == FALSE) {
                mNpc_SendEventPresentMailSex(&other_sex_best_friends[i], &types[i], animal_p,
                                             mNpc_GetAnimalSex(animal_p));
            }
    ...
    */
        ...
        if (pid != NULL && mLd_CheckThisLand(pid->land_name, pid->land_id) == TRUE) {
            for (i = 0; i < ANIMAL_NUM_MAX; i++) {
                if (mNpc_CheckFriendship(pid, animal) != -1 &&
                    mNpc_SendBirthdayCard(pid, player_no, &animal->id) == TRUE) {
                    res = TRUE;
                }
    ...
    static void mNpc_GetXmasCardData(Mail_c * mail, PersonalID_c * pid) {
        ...
        mHandbill_Load_HandbillFromRom(mail->content.text.split.header, &header_back_start,
                                       mail->content.text.split.footer, mail->content.text.split.body, 0xD7);
        mail->content.header_back_start = header_back_start;
        mail->content.font = mMl_FONT_RECV;
    ...
    static void mNpc_SetDefAnimalInfo(Animal_c * animal, mActor_name_t npc_id, u8 looks,
                                      mNpc_Default_Data_c * def_data) {
        mLd_land_info_c* land_info = Save_GetPointer(land_info);
    ...
    extern void mNpc_DestroyHouse_block(mFM_fg_c* fg, Anmhome_c* home) {
        ...
        if (bx < FG_BLOCK_X_NUM && bz < FG_BLOCK_Z_NUM && ux > 0 && ux < (UT_X_NUM - 1) && uz > 0 &&
            uz < (UT_Z_NUM - 1)) {
            mActor_name_t reserve_no = mFM_GetReseveName_com(home->block_x, home->block_z, fg);
    ...
    extern void mNpc_RenewalNpcRoom(s16 * wall_floor) {
        ...
        if (Save_Get(scene_no) == SCENE_COTTAGE_NPC) {
            ...
            if (island_room_ftr_num >= 2 && Common_Get(cur_island_house_p) != NULL) {
                mNpc_GetIslandWallFloorIdx(&island_wall, &island_floor, Common_Get(cur_island_house_p)->island.animal.id.npc_id);
                wall = island_wall;
    ...
    static mActor_name_t mNpc_DecideNpcFurniture(mFM_fg_data_c * *fg_data_table, mNpc_NpcList_c * npclist,
                                                 int fg_base_id) {
        ...
        if (n_players > 0) {
            for (i = 0; i < ANIMAL_NUM_MAX; i++) {
                if (mNpc_CheckFreeAnimalInfo(animal) == FALSE && i != ignored_idx) {
                    ...
                    for (j = 0; j < PLAYER_NUM; j++) {
                        if (mPr_NullCheckPersonalID(&priv_p->player_ID) == FALSE) {
                            if (mNpc_GetAnimalMemoryIdx(&priv_p->player_ID, animal->memories, ANIMAL_MEMORY_NUM) ==
                                -1) {
                                break;
                            }
        ...
        if (res1 > res0) {
            ...
            if (res1 > res0) {
                ...
                if (res1 > res0) {
                    ...
                    /* Finally, check the total number of animals in town with the same personality and pick the one
                     * with more */
    ...
    static int mNpc_SendGoodbyAnimalMailOne(Mail_c * mail, PersonalID_c * pid, int player_no,
                                            Anm_GoodbyMail_c* goodby_mail) {
        int free_space;
        mHm_hs_c* home;
        int res = FALSE;
    ...
    extern void mNpc_GetNpcWorldNameAnm(u8 * dst, AnmPersonalID_c * anm_id) {
        ...
        if (anm_id != NULL) {
            if (anm_id->npc_id != RSV_NO &&
                mNpc_GetCacheName(animal_name, anm_id->npc_id, mNpc_NAME_TYPE_NPC) != FALSE) {
                name = animal_name;
        ...
        if (items != NULL && col != NULL) {
            for (i = restrict_area; i < UT_Z_NUM - restrict_area; i++) {
                for (j = restrict_area; j < UT_X_NUM - restrict_area; j++) {
                    if (mNpc_CheckNpcSet_fgcol_hard(items[i * UT_X_NUM + j],
                                                    col[i * UT_X_NUM + j].data.unit_attribute) == TRUE) {
    ...
    extern int mNpc_GetMakeUtNuminBlock_area(int* ut_x, int* ut_z, int bx, int bz, int restrict_area) {
        ...
        if (fg_top != NULL && col_top != NULL) {
            for (i = restrict_area; i < UT_Z_NUM - restrict_area; i++) {
                for (j = restrict_area; j < UT_X_NUM - restrict_area; j++) {
                    if (mNpc_CheckNpcSet_fgcol(fg_top[i * UT_X_NUM + j],
    ...
    static void mNpc_UnlockTimerCountDown(mNpc_Talk_Info_c * talk_info) {
        if (talk_info != NULL && talk_info->unlock_timer > 0 &&
            talk_info->unlock_timer > (talk_info->reset_timer - 1000)) {
    ...
    extern void mNpc_SetIslandRoomFtr(Animal_c * animal) {
        ...
        if (animal != NULL) {
            ...
            // bcopy(&animal->anmuni.island_ftr, island_ftr_p->trade_list, mNpc_ISLAND_FTR_SAVE_NUM *
...
extern int mNpc_SetIslandFtr(PersonalID_c* pid, mActor_name_t ftr) {
    ...
    extern int mNpc_ReceiveHPMail(Mail_c * mail) {
        ...
        if (mail->header.sender.type == mMl_NAME_TYPE_PLAYER && private_idx >= 0 && private_idx < PLAYER_NUM) {
            int is_password = mMpswd_restore_code(mail->content.text.split.body, password);
...
/* @BUG - devs used && instead of || */
        ...
        if (mPO_get_keep_mail_sum() < mPO_MAIL_STORAGE_SIZE) {
            ...
            if (mMpswd_password_zuru_check(password) == FALSE && // checksum is valid
                ...
                if (password->npc_type == mMpswd_NPCTYPE_NORMAL) {
                    ...
                    //                              ||
                    //      mNpc_GetAnimalMemoryIdx(&Common_Get(now_private)->player_ID,
                    //      Save_Get(island).animal.memories,
                    //                              ANIMAL_MEMORY_NUM) != -1)) {
                    //     int looks = mNpc_GetLooks(target_npc_id);

                    //     if (mNpc_GetHit_cardE(password->hit_rate_index)) {
                    //         famicom_hit = TRUE;
                    //         mail_no = mNpc_CheckIslandAnimalTableNo(target_npc_id) == TRUE ? 0x3CA + looks : 0x3C4 +
                    //         looks; mNpc_GetNpcWorldNameTableNo(name_str, NPC_START | password->npc_code);
                    //         mNpc_GetNpcWorldNameTableNo(submenu_name_str, NPC_START | password->npc_code);
                    //         valid = TRUE;
                    //     }
                    // }
    ...
    static int mNpc_SendHPMailNum_user_password(PersonalID_c * pid, AnmPersonalID_c * anm_id,
    ...
    static int mNpc_SendHPMailNum_cardE_mini(PersonalID_c * pid, AnmPersonalID_c * anm_id,
                                             mMpswd_password_c * password) {
