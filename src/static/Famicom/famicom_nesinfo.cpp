    ...
    while (TRUE) {
        ...
        if (memcmp(data, NESTAG_VEQ, NESTAG_CMD_SIZE) == 0) {
            ...
            if (save_data != nullptr && mode == 1) {
                static const u32 cCheckMask = 1 << 30;

                if (((*(u32*)&save_data[4] & (1 << 30)) == 0)) {
                    if ((*(u32*)&save_data[4] & (1 << 31)) != 0) {
                        *(u32*)&save_data[4] = 0;
                    }

                    (*(u32*)&save_data[4]) |= cCheckMask;
                }

                if (((*(u32*)&save_data[4]) & bit) == 0) {
            ...
            if (nesrom_filename_ptrs != nullptr && rom_p != nullptr) {
                nesinfo_data_size = JKRReadResource(rom_p, 0x100000, (char*)nesrom_filename_ptrs[rom_id],
