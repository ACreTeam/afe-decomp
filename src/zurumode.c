...
static u8 zurumode_initialized;
static zuru_keycheck zuruKeyCheck;

static void zurucheck_init(zuru_keycheck* key_check) {
 ...
 */
static int zurucheck_key_check(zuru_keycheck* key_check, u32 controller) {
...
static s32 zurumode_update() {
    ...
    zurumode_flag = zurumode_now = (APPNMI_ZURUMODE_GET()) ? (APPNMI_ZURUMODE2_GET() ? 2 : 0) : 0;
...
static void zurumode_callback(void* param) {
    zurucheck_key_check(&zuruKeyCheck, (u32)(((padmgr*)param)->cur_pads[1].button));
...
static void zurumode_onFault(void* param) {
    if (zurumode_initialized) {
        zurucheck_key_check(&zuruKeyCheck, (u32)param);

        if (zuruKeyCheck.zurumode_enabled) {
            APPNMI_ZURUMODE_SET();
        } else {
            APPNMI_ZURUMODE_CLR();
        }
...
extern void zurumode_init(void) {
    zurumode_flag = 0;
    zurucheck_init(&zuruKeyCheck);
    zuruKeyCheck.zurumode_enabled = APPNMI_ZURUMODE_GET();
    padmgr_setClient2(zurumode_callback, &padmgr_class);
    zurumode_update();
    zurumode_onFaultCallback = zurumode_onFault;
    zurumode_initialized = TRUE;
}

extern void zurumode_cleanup(void) {
    zurumode_initialized = FALSE;
    zurumode_onFaultCallback = NULL;
