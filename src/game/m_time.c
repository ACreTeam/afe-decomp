...
const lbRTC_time_c mTM_rtcTime_zero_code = {
    // clang-format off
    0x00, 0x00, 0x00,
    0x00, 0x00, 0x00,
    0x0000
...
extern void mTM_renewal_renew_time() {
    ...
    if ((Common_Get(time.rtc_crashed) || Common_Get(time.rtc_enabled)) && ((renew_time->year != rtc_time->year) || (renew_time->month != rtc_time->month) ||
