...
extern void game_main(GAME* this) {
    ...
    // Don't play audio when reloading data between indoor/outdoor scenes
    if (this->game_dlf_no != GAME_DLFTBL_RELOAD_DATA) {
        mBGM_main(this);
    }
...
extern void game_resize_hyral(GAME* this, int size) {
    ...
    if (!mLd_CheckStartFlag() && max_size >= 0x18000) {
        max_size -= 0x18000;
    }
...
extern void game_ct(GAME* this, void (*init)(GAME*), GRAPH* graph, int dlf_no) {
    const size_t hryal_size = GAME_HYRAL_SIZE;
    GameAlloc* gamealloc = &this->gamealloc;
    
    gamePT = this;
    this->game_dlf_no = dlf_no;
    ...
    game_init_hyral(this, hryal_size);
    SetGameFrame(GAME_FRAME);
    __gfxprint_default_flags |= GFXPRINT_FLAG_HIGHRES;
    init(this);

    // Don't initialize audio when reloading data between indoor/outdoor scenes
    if (this->game_dlf_no != GAME_DLFTBL_RELOAD_DATA) {
        mBGM_init();
    }
...
extern void game_dt(GAME* this) {
    ...
    // Don't cleanup audio when reloading data between indoor/outdoor scenes
    if (this->game_dlf_no != GAME_DLFTBL_RELOAD_DATA) {
        mBGM_cleanup();
    }
...
extern void game_goto_next_game_reload_data(GAME* this) {
    GAME_GOTO_NEXT(this, reload_data, RELOAD_DATA);
}
