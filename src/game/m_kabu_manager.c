...
static void Kabu_decide_trade_market() {
    ...
    static f32* next_trade_market[Kabu_TRADE_MARKET_TYPE_NUM] = { next_trade_market_typeA, next_trade_market_typeB,
                                                                  next_trade_market_typeC, };

    int trade_market;
    f32* next_type_market;
    f32 chosen;
    int i;

    trade_market = Save_Get(kabu_price_schedule.trade_market);
#if VERSION == VER_GAEJ01_00
    next_type_market = next_trade_market[trade_market];
    chosen = fqrand(); /* [0.0f, 1.0f) */
#else
    chosen = fqrand();
    if (trade_market >= Kabu_TRADE_MARKET_TYPE_NUM) {
        trade_market = 0;
    }
    next_type_market = next_trade_market[trade_market];
#endif
...
static void Kabu_decide_price_schedule_typeC() {
    ...
    for (day = lbRTC_MONDAY; day <= lbRTC_SATURDAY; day++) {
        current_price *=
            (TRADE_MARKET_C_ADJUST_MULT_BASE + TRADE_MARKET_C_ADJUST_MULT_SPREAD * fqrand()); /* [80%, 95%) */
        *price++ = current_price;
    }
}
...
static void Kabu_decide_price_sunday() {
    ...
    Save_Set(kabu_price_schedule.daily_price[lbRTC_SUNDAY],
             price * TRADE_MARKET_SUNDAY_BASE_PRICE); /* [0.7, 1.3) * 100.0f = [70, 130) bells */
}
...
static void Kabu_decide_price_without_sunday() {
    static void (*process[])(void) = { Kabu_decide_price_schedule_typeA, Kabu_decide_price_schedule_typeB,
                                       Kabu_decide_price_schedule_typeC };
...
extern void Kabu_manager() {
    ...
    if (lbRTC_IsEqualDate(rtc_time->year, rtc_time->month, rtc_time->day, kabu_update_time->year,
                          kabu_update_time->month, kabu_update_time->day) == lbRTC_EQUAL) {
