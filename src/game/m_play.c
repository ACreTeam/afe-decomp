...
static u8 pr_gxbuf[0x140] ATTRIBUTE_ALIGN(32);
static u8 prbuf[2 * (SCREEN_WIDTH) * (SCREEN_HEIGHT) * sizeof(u32)] ATTRIBUTE_ALIGN(32); // 0x12C000
...
static void Game_play_change_scene_move_end(GAME_PLAY* play) {
    game_goto_next_game_reload_data(&play->game);
...
extern void play_cleanup(GAME* game) {
    ...
    mCsd_Set_EfbTextureBuffer_p(NULL, 0);
...
extern void play_init(GAME* game) {
    ...
    mCsd_Set_EfbTextureBuffer_p(prbuf, sizeof(prbuf));
    mSP_init_check_door();
}
...
static u32 copy_efb_to_texture_dl(u8* gxbuf, size_t gxbuf_size, u8* buf) {
    GXBeginDisplayList(gxbuf, gxbuf_size);
    ...
    // gGXCallDisplayList((*graph)++, pr_gxbuf, dl);
}
...
static void capture_display_data(GRAPH* graph, u8 mode) {
    Gfx* polydisp;
    Gfx* newdisp;
    ...
    switch (mode) {
        case mSM_CAPTURE_DISPLAY_MODE_SUBMENU:
            gSPDisplayList(NEXT_LIGHT_DISP, polydisp);
            gGXCallDisplayList(polydisp++, pr_gxbuf, copy_efb_to_texture_dl(pr_gxbuf, sizeof(pr_gxbuf), prbuf));
            break;
        case mSM_CAPTURE_DISPLAY_MODE_FILM:
            gSPDisplayList(NEXT_NEW2_DISP, polydisp);
            gGXCallDisplayList(polydisp++, pr_gxbuf, copy_efb_to_texture_dl(pr_gxbuf, sizeof(pr_gxbuf), prbuf));
            break;
    }
...
static int makeBumpTexture(GAME_PLAY* play, GRAPH* graph1, GRAPH* graph2) {
    ...
    if ((play->submenu.mode == mSM_MODE_PRERENDER_INIT) || (play->fb_mode == FBDEMO_MODE_CREATE)) {
        capture_display_data(graph1, play->submenu.capture_display_mode);
