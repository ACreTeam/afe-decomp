...
extern int mMmd_GetDisplayInfo(mActor_name_t item) {
    if (item >= FTR_START(FTR_DIN_TRIKERA_HEAD) && item <= FTR_END(FTR_DIN_TRILOBITE)) {
        ...
        if (mMmd_FossilInfo(fossil_idx) >= mMmd_DONATOR_PLAYER1 &&
            mMmd_FossilInfo(fossil_idx) <= mMmd_DONATOR_DELETED_PLAYER) {
        ...
        if (mMmd_ArtInfo(painting_idx) >= mMmd_DONATOR_PLAYER1 &&
            mMmd_ArtInfo(painting_idx) <= mMmd_DONATOR_DELETED_PLAYER) {
            return mMmd_DISPLAY_ALREADY_DONATED;
        } else {
            return mMmd_DISPLAY_CAN_DONATE;
        }
    } else if (item >= ITM_INSECT00 && item < ITM_INSECT_END) {
        int insect_idx;
        int valid = FALSE;

        /* ??? why check this again */
        if (item >= ITM_INSECT00 && item < ITM_INSECT_END) {
            valid = TRUE;
        }

        insect_idx = valid != FALSE ? item - ITM_INSECT_START : 0;
        if (mMmd_InsectInfo(insect_idx) >= mMmd_DONATOR_PLAYER1 &&
            mMmd_InsectInfo(insect_idx) <= mMmd_DONATOR_DELETED_PLAYER) {
            return mMmd_DISPLAY_ALREADY_DONATED;
        } else {
            return mMmd_DISPLAY_CAN_DONATE;
        }
    } else if (item >= ITM_FISH00 && item < ITM_FISH_END) {
        int fish_idx = item - ITM_FISH_START;
...
extern int mMmd_RequestMuseumDisplay(mActor_name_t item) {
    if (mLd_PlayerManKindCheck() == FALSE) {
        if (item >= FTR_START(FTR_DIN_TRIKERA_HEAD) && item <= FTR_END(FTR_DIN_TRILOBITE)) {
            ...
            if (mMmd_FishInfo(fish_idx) < mMmd_DONATOR_PLAYER1 ||
                mMmd_FishInfo(fish_idx) > mMmd_DONATOR_DELETED_PLAYER) {
    ...
    if (fossil_fg != NULL) {
        for (fossil_idx = 0; fossil_idx < mMmd_FOSSIL_NUM; fossil_idx++) {
            ...
            if (mMmd_FossilInfo(fossil_idx)) {
                ...
                if (valid) {
                    mActor_name_t fossil_ftr_idx =
                        FTR_START(FTR_DIN_TRIKERA_HEAD) + FTR_NO_2_IDX(fossil_idx);
                    item = fossil_ftr_idx;
                }
